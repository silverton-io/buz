locals {
  ...
  metadata_extraction_params = "{valid:.valid,vendor:.vendor,namespace:.namespace,version:.version}"
  s3_dynamic_prefix   = "isValid=!{partitionKeyFromQuery:valid}/vendor=!{partitionKeyFromQuery:vendor}/namespace=!{partitionKeyFromQuery:namespace}/version=!{partitionKeyFromQuery:version}/year=!{timestamp:yyyy}/month=!{timestamp:MM}/day=!{timestamp:dd}/"
}
